        ...
        try:
            if trainer:
                ...
                if not reset_response.success:
                    return {"success": False, "error": f"Server reset failed: {reset_response.error}"}
...
async def export_model():
    ...
    try:
        ...
        for i in range(60):
            ...
            print(f"[Export] Poll {i+1}/60 - Finished: {status.is_finished}, Error: {status.error}")
    ...
    try:
        while True:
            ...
            try:
                ...
                # Debug logging
                print(f"[WebSocket] Phase: {status.phase}, Export: {status.export_entries_processed}/{status.export_entries_total}")
